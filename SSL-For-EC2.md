...
```
**SSL Steps (run on EC2 after DNS is pointed)**

```
# 1. Install Certbot
sudo apt update && sudo apt install certbot python3-certbot-nginx -y

# 2. Update system Nginx config (paste the config above)
sudo nano /etc/nginx/sites-available/flowboard
sudo nginx -t && sudo systemctl restart nginx

# 3. Get SSL certificate (auto-configures Nginx for HTTPS)
sudo certbot --nginx -d shubhamx18.live -d www.shubhamx18.live

# 4. Rebuild frontend image with HTTPS URLs then restart
docker compose down
docker compose pull frontend
docker compose up -d

# 5. Test
curl https://knoxcloud.tech/api/health
